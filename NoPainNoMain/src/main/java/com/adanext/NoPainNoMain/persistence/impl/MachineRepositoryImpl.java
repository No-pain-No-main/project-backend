...
public class MachineRepositoryImpl implements MachineRepository {
  ...
  public Optional<Machine> findById(Integer id) {
    return repository.findById(id).map(MachineMapper::toDomain);
  }

  @Override
  public Optional<Machine> findByName(String name) {
    return repository.findByName(name).map(MachineMapper::toDomain);
  }

  @Override
  public List<Machine> findAll() {
    return repository.findAll().stream().map(MachineMapper::toDomain).collect(Collectors.toList());
  }
